...
interface ProgramArticlesSectionProps {
  ...
  title: string;        // e.g., 'Công nghệ thông tin'
}

const ProgramArticlesSection: React.FC<ProgramArticlesSectionProps> = ({ categorySlug, title }) => {
  ...
  useEffect(() => {
    ...
    const fetchArticles = async () => {
      try {
        ...
        const url = `${base}/articles/by-category/${categorySlug}?${params.toString()}`;
        const res = await fetch(url, { cache: 'no-store' });
        if (!res.ok) throw new Error('Failed to fetch');
        const data = await res.json();
        if (isMounted) {
          const list: Article[] = data?.data?.articles || [];
          setArticles(list);
          setError(null);
        }
      } catch (err: any) {
    ...
    <section aria-labelledby="program-articles-title" className="w-[92%] md:w-[80%] mx-auto py-8 md:py-12">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 id="program-articles-title" className="text-xl md:text-2xl font-bold text-slate-900">Bài viết liên quan ngành {title}</h2>
      ...
      {loading ? (
        ...
        <ArticlesGrid articles={articles} columns={{ mobile: 1, tablet: 2, desktop: 4 }} />
      )}
    </section>
  );
};
