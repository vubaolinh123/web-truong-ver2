/**
 * Training Cooperation Page
 * Displays paginated grid of news articles from the "Hợp tác đào tạo" category
 */
...
const TrainingCooperationPagination = dynamic(() => import('./components/TrainingCooperationPagination'), {
  loading: () => (
...
const CATEGORY_SLUG = 'hop-tac-dao-tao';

// Page props interface
interface TrainingCooperationPageProps {
...
export const metadata: Metadata = {
  title: `Hợp tác đào tạo - ${SITE_NAME}`,
  description:
    'Các chương trình hợp tác đào tạo với các tổ chức, doanh nghiệp và đối tác trong và ngoài nước',
  alternates: { canonical: `${SITE_URL}/hop-tac-dao-tao` },
  openGraph: {
    title: `Hợp tác đào tạo - ${SITE_NAME}`,
    description:
      'Các chương trình hợp tác đào tạo với các tổ chức, doanh nghiệp và đối tác trong và ngoài nước',
    url: `${SITE_URL}/hop-tac-dao-tao`,
    type: 'website',
    locale: 'vi_VN',
  ...
  twitter: {
    ...
    title: `Hợp tác đào tạo - ${SITE_NAME}`,
    description: 'Các chương trình hợp tác đào tạo với các tổ chức, doanh nghiệp và đối tác trong và ngoài nước',
    images: [`${SITE_URL}/images/logo.png`],
  },
};

// Data fetching function
async function getTrainingCooperationArticles(params: {
  page?: number;
  sort?: string;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch training cooperation articles');
    }
    ...
    return {
      ...
      categoryName: data?.data?.category?.name || 'Hợp tác đào tạo'
    };
  } catch (error) {
    console.error('Error fetching training cooperation articles:', error);
    return {
      articles: [],
      total: 0,
      totalPages: 1,
      categoryName: 'Hợp tác đào tạo'
    };
  }
}
...
const structuredData = [
  {
    ...
    '@type': 'CollectionPage',
    name: `Hợp tác đào tạo - ${SITE_NAME}`,
    description: 'Các chương trình hợp tác đào tạo với các tổ chức, doanh nghiệp và đối tác trong và ngoài nước',
    url: `${SITE_URL}/hop-tac-dao-tao`,
    isPartOf: {
      '@type': 'WebSite',
...
export default async function TrainingCooperationPage({ searchParams }: TrainingCooperationPageProps) {
  const params = await searchParams;
  const currentPage = parseInt(params.page || '1');
  const sort = params.sort || 'newest';

  // Fetch training cooperation articles
  const { articles, total, totalPages, categoryName } = await getTrainingCooperationArticles({
    page: currentPage,
    sort
  });
  ...
  const breadcrumbItems = [
    ...
    { label: 'Hợp tác đào tạo', href: '/hop-tac-dao-tao', current: true }
  ];
