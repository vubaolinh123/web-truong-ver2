 ...
 * Displays paginated grid of news articles from the "Hợp tác quốc tế" category
 */
...
const InternationalCooperationPagination = dynamic(() => import('./components/InternationalCooperationPagination'), {
  loading: () => (
...
const CATEGORY_SLUG = 'hop-tac-quoc-te';

// Page props interface
interface InternationalCooperationPageProps {
...
export const metadata: Metadata = {
  title: `Hợp tác quốc tế - ${SITE_NAME}`,
  description:
    'Thông tin về các hoạt động hợp tác quốc tế, trao đổi học thuật và quan hệ đối tác toàn cầu',
  alternates: { canonical: `${SITE_URL}/hop-tac-quoc-te` },
  openGraph: {
    title: `Hợp tác quốc tế - ${SITE_NAME}`,
    description:
      'Thông tin về các hoạt động hợp tác quốc tế, trao đổi học thuật và quan hệ đối tác toàn cầu',
    url: `${SITE_URL}/hop-tac-quoc-te`,
    type: 'website',
    locale: 'vi_VN',
  ...
  twitter: {
    ...
    title: `Hợp tác quốc tế - ${SITE_NAME}`,
    description: 'Thông tin về các hoạt động hợp tác quốc tế, trao đổi học thuật và quan hệ đối tác toàn cầu',
    images: [`${SITE_URL}/images/logo.png`],
  },
};

// Data fetching function
async function getInternationalCooperationArticles(params: {
  page?: number;
  sort?: string;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch international cooperation articles');
    }
    ...
    return {
      ...
      categoryName: data?.data?.category?.name || 'Hợp tác quốc tế'
    };
  } catch (error) {
    console.error('Error fetching international cooperation articles:', error);
    return {
      articles: [],
      total: 0,
      totalPages: 1,
      categoryName: 'Hợp tác quốc tế'
    };
  }
}
...
const structuredData = [
  {
    ...
    '@type': 'CollectionPage',
    name: `Hợp tác quốc tế - ${SITE_NAME}`,
    description: 'Thông tin về các hoạt động hợp tác quốc tế, trao đổi học thuật và quan hệ đối tác toàn cầu',
    url: `${SITE_URL}/hop-tac-quoc-te`,
    isPartOf: {
      '@type': 'WebSite',
...
export default async function InternationalCooperationPage({ searchParams }: InternationalCooperationPageProps) {
  const params = await searchParams;
  const currentPage = parseInt(params.page || '1');
  const sort = params.sort || 'newest';

  // Fetch international cooperation articles
  const { articles, total, totalPages, categoryName } = await getInternationalCooperationArticles({
    page: currentPage,
    sort
  });
  ...
  const breadcrumbItems = [
    ...
    { label: 'Hợp tác quốc tế', href: '/hop-tac-quoc-te', current: true }
  ];
          ...
          <div className="w-full max-w-[92%] mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            {totalPages > 1 && (
              <Suspense fallback={
                ...
                <InternationalCooperationPagination
                  currentPage={currentPage}
                  totalPages={totalPages}
                  sort={sort}
                />
