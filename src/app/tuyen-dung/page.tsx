/**
 * Recruitment Page
 * Displays paginated grid of news articles from the "Tuyển dụng" category
 */
...
const RecruitmentPagination = dynamic(() => import('./components/RecruitmentPagination'), {
  loading: () => (
...
export const metadata: Metadata = {
  ...
  description:
    'Thông tin tuyển dụng, cơ hội việc làm và kết nối doanh nghiệp cho sinh viên và cựu sinh viên',
  alternates: { canonical: `${SITE_URL}/tuyen-dung` },
  openGraph: {
    title: `Tuyển dụng - ${SITE_NAME}`,
    description:
      'Thông tin tuyển dụng, cơ hội việc làm và kết nối doanh nghiệp cho sinh viên và cựu sinh viên',
  ...
  twitter: {
    ...
    title: `Tuyển dụng - ${SITE_NAME}`,
    description: 'Thông tin tuyển dụng, cơ hội việc làm và kết nối doanh nghiệp cho sinh viên và cựu sinh viên',
    images: [`${SITE_URL}/images/logo.png`],
  },
};

// Data fetching function
async function getRecruitmentArticles(params: {
  page?: number;
  sort?: string;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch recruitment articles');
    }
    ...
    return {
      ...
      categoryName: data?.data?.category?.name || 'Tuyển dụng'
    };
  } catch (error) {
    console.error('Error fetching recruitment articles:', error);
    return {
      articles: [],
      total: 0,
      totalPages: 1,
...
export default async function RecruitmentPage({ searchParams }: RecruitmentPageProps) {
  ...
  const { articles, total, totalPages, categoryName } = await getRecruitmentArticles({
    page: currentPage,
    sort
  });
  ...
  const breadcrumbItems = [
    ...
    { label: 'Tuyển dụng', href: '/tuyen-dung', current: true }
  ];
