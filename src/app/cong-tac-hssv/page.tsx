 ...
 * Displays paginated grid of news articles from the "Công tác HSSV" category
 */
...
const StudentAffairsPagination = dynamic(() => import('./components/StudentAffairsPagination'), {
  loading: () => (
...
const CATEGORY_SLUG = 'cong-tac-hssv';

// Page props interface
interface StudentAffairsPageProps {
...
export const metadata: Metadata = {
  title: `Công tác học sinh sinh viên - ${SITE_NAME}`,
  description:
    'Thông tin về các hoạt động, chính sách và hỗ trợ dành cho học sinh sinh viên',
  alternates: { canonical: `${SITE_URL}/cong-tac-hssv` },
  openGraph: {
    title: `Công tác học sinh sinh viên - ${SITE_NAME}`,
    description:
      'Thông tin về các hoạt động, chính sách và hỗ trợ dành cho học sinh sinh viên',
    url: `${SITE_URL}/cong-tac-hssv`,
    type: 'website',
    locale: 'vi_VN',
  ...
  twitter: {
    ...
    title: `Công tác học sinh sinh viên - ${SITE_NAME}`,
    description: 'Thông tin về các hoạt động, chính sách và hỗ trợ dành cho học sinh sinh viên',
    images: [`${SITE_URL}/images/logo.png`],
  },
};

// Data fetching function
async function getStudentAffairsArticles(params: {
  page?: number;
  sort?: string;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch student affairs articles');
    }
    ...
    return {
      ...
      categoryName: data?.data?.category?.name || 'Công tác HSSV'
    };
  } catch (error) {
    console.error('Error fetching student affairs articles:', error);
    return {
      articles: [],
      total: 0,
      totalPages: 1,
      categoryName: 'Công tác HSSV'
    };
  }
}
...
const structuredData = [
  {
    ...
    '@type': 'CollectionPage',
    name: `Công tác học sinh sinh viên - ${SITE_NAME}`,
    description: 'Thông tin về các hoạt động, chính sách và hỗ trợ dành cho học sinh sinh viên',
    url: `${SITE_URL}/cong-tac-hssv`,
    isPartOf: {
      '@type': 'WebSite',
...
export default async function StudentAffairsPage({ searchParams }: StudentAffairsPageProps) {
  const params = await searchParams;
  const currentPage = parseInt(params.page || '1');
  const sort = params.sort || 'newest';

  // Fetch student affairs articles
  const { articles, total, totalPages, categoryName } = await getStudentAffairsArticles({
    page: currentPage,
    sort
  });
  ...
  const breadcrumbItems = [
    ...
    { label: 'Công tác học sinh sinh viên', href: '/cong-tac-hssv', current: true }
  ];
