/**
 * Training Regulations Page
 * Displays paginated grid of news articles from the "Quy chế đào tạo" category
 */
...
const TrainingRegulationsPagination = dynamic(() => import('./components/TrainingRegulationsPagination'), {
  loading: () => (
...
const CATEGORY_SLUG = 'quy-che-dao-tao';

// Page props interface
interface TrainingRegulationsPageProps {
...
export const metadata: Metadata = {
  title: `Quy chế đào tạo - ${SITE_NAME}`,
  description:
    'Các quy định, quy chế và văn bản hướng dẫn về đào tạo và quản lý sinh viên',
  alternates: { canonical: `${SITE_URL}/quy-che-dao-tao` },
  openGraph: {
    title: `Quy chế đào tạo - ${SITE_NAME}`,
    description:
      'Các quy định, quy chế và văn bản hướng dẫn về đào tạo và quản lý sinh viên',
    url: `${SITE_URL}/quy-che-dao-tao`,
    type: 'website',
    locale: 'vi_VN',
  ...
  twitter: {
    ...
    title: `Quy chế đào tạo - ${SITE_NAME}`,
    description: 'Các quy định, quy chế và văn bản hướng dẫn về đào tạo và quản lý sinh viên',
    images: [`${SITE_URL}/images/logo.png`],
  },
};

// Data fetching function
async function getTrainingRegulationsArticles(params: {
  page?: number;
  sort?: string;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch training regulations articles');
    }
    ...
    return {
      ...
      categoryName: data?.data?.category?.name || 'Quy chế đào tạo'
    };
  } catch (error) {
    console.error('Error fetching training regulations articles:', error);
    return {
      articles: [],
      total: 0,
      totalPages: 1,
      categoryName: 'Quy chế đào tạo'
    };
  }
}
...
const structuredData = [
  {
    ...
    '@type': 'CollectionPage',
    name: `Quy chế đào tạo - ${SITE_NAME}`,
    description: 'Các quy định, quy chế và văn bản hướng dẫn về đào tạo và quản lý sinh viên',
    url: `${SITE_URL}/quy-che-dao-tao`,
    isPartOf: {
      '@type': 'WebSite',
...
export default async function TrainingRegulationsPage({ searchParams }: TrainingRegulationsPageProps) {
  const params = await searchParams;
  const currentPage = parseInt(params.page || '1');
  const sort = params.sort || 'newest';

  // Fetch training regulations articles
  const { articles, total, totalPages, categoryName } = await getTrainingRegulationsArticles({
    page: currentPage,
    sort
  });
  ...
  const breadcrumbItems = [
    ...
    { label: 'Quy chế đào tạo', href: '/quy-che-dao-tao', current: true }
  ];
