/**
 * Văn bản quy chế & quy định Pagination Component
 * Pagination controls for van ban quy che page
 */
...
interface VanBanPaginationProps {
  currentPage: number;
  totalPages: number;
  sort?: string;
}

const VanBanPagination: React.FC<VanBanPaginationProps> = ({
  currentPage,
  totalPages,
  sort = 'newest'
}) => {
  const buildPageUrl = (page: number) => {
    ...
    return `/van-ban-quy-che-quy-dinh${params.toString() ? `?${params.toString()}` : ''}`;
  };
...
export default VanBanPagination;
