/**
 * Văn bản quy chế & quy định Page
 * Displays paginated grid of articles from the "van-ban-quy-che-quy-dinh" category
 */
...
const VanBanPagination = dynamic(() => import('./components/VanBanPagination'), {
  loading: () => (
...
const CATEGORY_SLUG = 'van-ban-quy-che-quy-dinh';

// Page props interface
interface VanBanPageProps {
...
export const metadata: Metadata = {
  title: `Văn bản quy chế & quy định - ${SITE_NAME}`,
  description:
    'Tổng hợp các văn bản quy chế, quy định và các tài liệu pháp lý liên quan đến hoạt động đào tạo và quản lý của trường',
  alternates: { canonical: `${SITE_URL}/van-ban-quy-che-quy-dinh` },
  openGraph: {
    title: `Văn bản quy chế & quy định - ${SITE_NAME}`,
    description:
      'Tổng hợp các văn bản quy chế, quy định và các tài liệu pháp lý liên quan đến hoạt động đào tạo và quản lý của trường',
    url: `${SITE_URL}/van-ban-quy-che-quy-dinh`,
    type: 'website',
    locale: 'vi_VN',
  ...
  twitter: {
    ...
    title: `Văn bản quy chế & quy định - ${SITE_NAME}`,
    description: 'Tổng hợp các văn bản quy chế, quy định và các tài liệu pháp lý liên quan đến hoạt động đào tạo và quản lý của trường',
    images: [`${SITE_URL}/images/logo.png`],
  },
};

// Data fetching function
async function getVanBanArticles(params: {
  page?: number;
  sort?: string;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch van ban articles');
    }
    ...
    return {
      ...
      categoryName: data?.data?.category?.name || 'Văn bản quy chế & quy định'
    };
  } catch (error) {
    console.error('Error fetching van ban articles:', error);
    return {
      articles: [],
      total: 0,
      totalPages: 1,
      categoryName: 'Văn bản quy chế & quy định'
    };
  }
}
...
const structuredData = [
  {
    ...
    '@type': 'CollectionPage',
    name: `Văn bản quy chế & quy định - ${SITE_NAME}`,
    description: 'Tổng hợp các văn bản quy chế, quy định và các tài liệu pháp lý liên quan đến hoạt động đào tạo và quản lý của trường',
    url: `${SITE_URL}/van-ban-quy-che-quy-dinh`,
    isPartOf: {
      '@type': 'WebSite',
...
export default async function VanBanQuyCheQuyDinhPage({ searchParams }: VanBanPageProps) {
  const params = await searchParams;
  const currentPage = parseInt(params.page || '1');
  const sort = params.sort || 'newest';

  // Fetch articles
  const { articles, total, totalPages, categoryName } = await getVanBanArticles({
    page: currentPage,
    sort
  });
  ...
  const breadcrumbItems = [
    ...
    { label: 'Văn bản quy chế & quy định', href: '/van-ban-quy-che-quy-dinh', current: true }
  ];
