/**
 * Study Abroad Page
 * Displays paginated grid of news articles from the "Du học" category
 */
...
const StudyAbroadPagination = dynamic(() => import('./components/StudyAbroadPagination'), {
  loading: () => (
...
export const metadata: Metadata = {
  title: `Du học - ${SITE_NAME}`,
  description:
    'Thông tin về các chương trình du học, học bổng quốc tế và cơ hội học tập tại nước ngoài',
  alternates: { canonical: `${SITE_URL}/du-hoc` },
  openGraph: {
    title: `Du học - ${SITE_NAME}`,
    description:
      'Thông tin về các chương trình du học, học bổng quốc tế và cơ hội học tập tại nước ngoài',
  ...
  twitter: {
    ...
    title: `Du học - ${SITE_NAME}`,
    description: 'Thông tin về các chương trình du học, học bổng quốc tế và cơ hội học tập tại nước ngoài',
    images: [`${SITE_URL}/images/logo.png`],
  },
};

// Data fetching function
async function getStudyAbroadArticles(params: {
  page?: number;
  sort?: string;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch study abroad articles');
    }
    ...
    return {
      ...
      categoryName: data?.data?.category?.name || 'Du học'
    };
  } catch (error) {
    console.error('Error fetching study abroad articles:', error);
    return {
      articles: [],
      total: 0,
      totalPages: 1,
...
export default async function StudyAbroadPage({ searchParams }: StudyAbroadPageProps) {
  ...
  // Fetch study abroad articles
  const { articles, total, totalPages, categoryName } = await getStudyAbroadArticles({
    page: currentPage,
    sort
  });
  ...
  const breadcrumbItems = [
    ...
    { label: 'Du học', href: '/du-hoc', current: true }
  ];
