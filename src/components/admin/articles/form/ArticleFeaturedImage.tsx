...
import { Upload, Image as ImageIcon, X, Camera, AlertTriangle } from 'lucide-react';
...
const ArticleFeaturedImage: React.FC<ArticleFeaturedImageProps> = ({
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      if (response.status === 'success' && response.data.url) {
        ...
        toast.success('Tải lên hình ảnh thành công!', { id: toastId });
      } else {
        const message = (response as { message?: string }).message || 'Lỗi không xác định trong quá trình tải lên';
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    const toastId = toast.loading('Đang xóa vĩnh viễn ảnh...');

    try {
      const filename = featuredImage.split('/').pop();
      if (!filename) throw new Error('Invalid image path');

      const response = await articlesApi.forceDeleteImage(filename);

      if (response.status === 'success') {
        toast.success('Đã xóa vĩnh viễn tệp ảnh.', { id: toastId });
        setPreviewUrl(null);
        onImageChange(null);
        ...
        const message = (response as { message?: string }).message || 'Lỗi không xác định';
        throw new Error(message);
      }
    } catch (err: unknown) {
      const errorMessage = err instanceof Error ? err.message : 'Không thể xóa vĩnh viễn ảnh.';
      toast.error(errorMessage, { id: toastId });
    } finally {
      setIsDeleting(false);
  ...
  return (
    <>
      ...
      <ConfirmationModal
        ...
        title="Xác nhận Xóa Vĩnh viễn Ảnh"
        message="Hành động này sẽ <strong>xóa vĩnh viễn</strong> tệp ảnh này khỏi máy chủ. <strong>Tất cả các bài viết khác</strong> (nếu có) đang sử dụng ảnh này cũng sẽ bị mất ảnh đại diện. Hành động này <strong>không thể hoàn tác</strong>. Bạn có chắc chắn muốn tiếp tục?"
